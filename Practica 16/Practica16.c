...
int main()
{
    ...
    struct jugador jugadores[5];//maximo 5 jugadores
    int opcion = -1;//-1 para que entre en el bucle

    for (int i=0; i<5; i++){
        printf("Introduce el nombre del jugador %d\n",i+1);
        gets(jugadores[i].nombre);//el gets guarda el salto de linea por lo que toca borrar buffer
    ...
    while (opcion != 0){
        ...
        if (opcion == 2){
            ...
            if (encontrado == 0){//si ya no se encuentra se imprime el mensaje
                printf("Jugador no encontrado\n");
            }
        }
        if (opcion == 3){
            float mayor_altura = jugadores[0].altura; //se le asigna el primer elemento de las alturas del struct
